...
std::string vfs::escape(std::string_view name, bool escape_slash)
{
	...
	for (usz i = 0, s = name.size(); i < s; i++)
	{
		...
		else if (static_cast<uchar>(name[i]) >= 0x80)
		{
			if (const usz size = utf8_sequence_size(name, i))
			{
				utf8_left = size - 1;
			}
			else
			{
				...
				result += reinterpret_cast<const char*>(u8"％");
				result += hex[static_cast<uchar>(name[i]) >> 4];
				result += hex[static_cast<uchar>(name[i]) & 0xf];
				continue;
			}
		}
