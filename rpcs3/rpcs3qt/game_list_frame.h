...
public:
	...
	struct ContentInfo
	{
		...
		std::set<std::string> removedDiscList; // Filled in by RemoveContentList(), BatchRemoveContentLists()
	};

public Q_SLOTS:
	void BatchCreateCPUCaches(const std::vector<game_info>& games = {}, bool is_interactive = false);
